...
namespace UserServiceTest
{
    ...
    public class UserServiceTest
    {
        ...
        public void TestLogout_ToeknShouldBeNil_AfetrLogout()
        {
            ...
            //Setup
            string token = "Token565#$%^";
            // _redisServiceMock.Setup(x => x.InsertData(It.IsAny<IDatabase>(), It.Is<string>(x => x.Equals(user.Email)), It.Is<String>(x => x.Equals(token))));

            _redisServiceMock.Setup(x => x.ReadData(It.IsAny<IDatabase>(), It.Is<string>(x => x.Equals(user.Email)))).Returns(token);
            _redisServiceMock.Setup(x => x.DeleteData(It.IsAny<IDatabase>(), It.Is<string>(x => x.Equals(user.Email))));


            // _redisServiceMock.Setup(x => x.RedisConnection()).Returns((_database);
        ...
        public async Task TestDeleteUser__UserShouldBeDeleted()
        {
            ...
            _redisServiceMock.Setup(x => x.ReadData(It.IsAny<IDatabase>(), It.Is<string>(x => x.Equals(user.Email)))).Returns(token);
            _redisServiceMock.Setup(x => x.DeleteData(It.IsAny<IDatabase>(), It.Is<string>(x => x.Equals(user.Email))));
            _userServiceMock.Setup(x => x.GetByEmail(It.Is<string>(x => x.Equals(user.Email)))).Returns(user);
            //Assert
